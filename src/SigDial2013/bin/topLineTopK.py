...
def main(argv):
	...
	for session in sessions:
		...
		for turn_index,(log_turn,label_turn) in enumerate(session):
			...
			#get the correct one
			rank, correct_slu = getSummary.getCorrectSLUHyp(log_turn, label_turn)
			if rank > topK or correct_slu==None: #do nothing
				continue
			
			#replace slu_hyp with correct one
			slu_hyp = getSummary.formatSlottoLive(correct_slu, rank)
