...
def extractScoreRequest():
	data = ['dstc2_test']
	
	score = []
	
	modes = ["asr_act_score","rawscore","noasr","noid","noscore","nosystem","noslu"]
	...
	for mode in modes:
		for test in data:
			#for k, L in enumerate([range(43,45)]):
			for k, L in enumerate([range(70,73)]):
				filename = "res/2waymodel_"+mode+"_"+test+"_score.txt"
				lines = fio.readfile(filename)
